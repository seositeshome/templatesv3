...
const saveTable = async (event) => {
  ...
  const generateMainTable = async (tableName) => {

    const { records } = await fetch(`https://api.seositeshome.com/tables/${tableName}settings`, {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json',
      },
    ...
    theadtr.innerHTML = ''
    const res = await fetch(`https://api.seositeshome.com/tables/${tableName}`, {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json',
      },
  ...
  const generateSettingTable = async (table) => {
    ...
    const { records } = await fetch(`https://api.seositeshome.com/tables/${table}settings`, {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json',
      },
  ...
  const runScript1 = () => {
    ...
    document.querySelectorAll('#tableData td').forEach(function (cell) {
  ...
  const runScript2 = () => {
    ...
    document.querySelectorAll('#tableData tbody td').forEach(function (cell) {
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    if(show ==='settings'){
        generateSettingTable(table)
    }
    else{
        generateMainTable(table)
    }
