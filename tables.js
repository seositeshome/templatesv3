...
const generateMainTable = async (tableName, token) => {
    ...
    records = [{ columnName: 'created', name: 'created' }, ... records.sort((a, b) => {return posA - posB;})]
        ...
        const updatedItems = Array.from(changedRows).map(row => {
            ...
            if (toDelete) {
                ...
                return {
                    ...
                    fkey: tds[7].querySelector('#foreignKey').value,
                    ftable: tds[7].querySelector('#foreignTable').value
                };
            }
        });
